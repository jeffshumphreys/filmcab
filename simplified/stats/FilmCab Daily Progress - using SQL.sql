SELECT 
  ...
  (SELECT sum(row_count)    AS "How many rows" FROM (SELECT table_name, 
    (SELECT n_live_tup FROM pg_stat_user_tables WHERE relname = table_name AND schemaname = 'simplified') AS row_count FROM information_schema. tables WHERE table_schema = 'simplified') AS "How many rows"),
