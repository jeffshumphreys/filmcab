<#
 #    FilmCab Daily morning batch run process:
 #
...
"@
    ...
    if ($CAPTURE_SNAPSHOT) {
        ...
        Invoke-Sql "TRUNCATE TABLE torrents_staged_snapshot"|Out-Null; # This is a staging table into "torrents".  Note that the the primary key is allowed to advance; the truncate command does not reset it to 0.  Some sort of history kept in the master table for now.
        ...
        foreach ($torrent in $torrents) {
            $Script:InsertTargetTableScript = "INSERT INTO torrents_staged_snapshot (
    ...
    if ($MERGE_STAGE_TO_MASTER) {
        ...
        $mergeSQL = @"
        MERGE INTO torrents_snapshots tgt
        USING
            ...
            FROM torrents_staged_snapshot new_src FULL JOIN torrents_snapshots old_tgt ON new_src.name = old_tgt.name) src
