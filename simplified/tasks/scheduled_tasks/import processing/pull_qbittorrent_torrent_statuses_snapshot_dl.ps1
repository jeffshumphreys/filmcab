<#
 #    FilmCab Daily morning batch run process: Fastest pull: only active downloads.
 #
...
"@
    ...
    DisplayTimePassed ("Beginning Code...")
    $CAPTURE_SNAPSHOT          = $true
    $MERGE_STAGE_TO_MASTER     = $false
    $ROLL_UP_IDENTICAL_PERIODS = $false
    $SPLIT_OUT_VALUE_HIST      = $false

    $torrentsStagedLoadBatchId = 1 # Don't eat up all the batch ids # [Int64](Get-SqlValue("SELECT nextval('torrents_staged_load_batch_id')")) # NOTE: once fetched, even in a transaction, can't recover that number unless you reset the sequence to this number.
    ...
    if ($CAPTURE_SNAPSHOT) {
        ...
        $torrents = Get-QbtTorrent $connectedAPISession -Filter "downloading" # -Limit 2
        ...
        Invoke-Sql "TRUNCATE TABLE torrents_staged_snapshot_dl"|Out-Null; # This is a staging table into "torrents".  Note that the the primary key is allowed to advance; the truncate command does not reset it to 0.  Some sort of history kept in the master table for now.
        ...
        foreach ($torrent in $torrents|Where-Object State -eq 'downloading') {
            $Script:InsertTargetTableScript = "INSERT INTO torrents_staged_snapshot_dl (
    ...
    if ($MERGE_STAGE_TO_MASTER) {
        ...
        $mergeSQL = @"
        MERGE INTO torrents_snapshots_dl tgt
        USING
            ...
            FROM torrents_staged_snapshot_dl new_src FULL JOIN torrents_snapshots_dl old_tgt ON new_src.name = old_tgt.name) src
    ...
    /**/
        WHEN MATCHED AND src.tgt_now_missing AND NOT src.src_is_new THEN -- See, these no longer exist in the qbittorrent app, so we keep that they were, AND UPDATE NOT WHEN they were removed FROM qbittorrent, but WHEN we detected they were removed.
            UPDATE SET
                ...
                original_load_batch_id                   = tgt.load_batch_id,
                load_batch_id                            = $torrentsStagedLoadBatchId,
                original_load_ batch_timestamp            = tgt.load_batch_timestamp,
