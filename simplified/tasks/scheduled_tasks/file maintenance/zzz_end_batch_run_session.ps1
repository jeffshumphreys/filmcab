...
if ($null -ne $state_of_session -and $state_of_session -isnot [String] -and $state_of_session.Count -eq 1) {
    if ( $state_of_session -is [System.Data.DataRow]) { # Out-SqlToDataset returns an object if only one row, hence
        Invoke-Sql "UPDATE batch_run_sessions_v SET running = NULL, session_ending_script = '$ScriptName', ended = CURRENT_TIMESTAMP, caller_ending = '$Script:Caller' WHERE running" | Out-Null
    } else {
...
elseif ($null -ne $state_of_session -and $state_of_session -isnot [String] -and $state_of_session.Table.Count -gt 2) {                         
    ...
    Show-Error -message "ERROR: More than one session marked active: Query was 'SELECT batch_run_session_id, started FROM batch_run_sessions_v WHERE running', STOPPING!" -exitcode 4
}                                                                                                                                                                         
