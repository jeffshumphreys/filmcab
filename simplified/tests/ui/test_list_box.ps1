...
$AddFoldersToSeenOffline = WhileReadSql "SELECT directory_path, useful_part_of_directory_path, parent_directory, folder FROM directories_ext_v dev WHERE search_path_id = $searchDirectoryId AND directory_depth >= 1 ORDER BY directory_depth, directory_path"
